	...
	public string AssembleSystemPrompt()
	{
		...
		if (memoryManager.memories.Count > 0)
		{
			...
			foreach (var memory in memoryManager.memories)
			{
				...
				prompt += "', time in minutes = " + memory.timeStamp.ToString("0"); // Truncate to nearest whole number
				prompt += " and atlas grid coordinate = " + memory.gridLocation;
		...
		prompt += "\nReplace ExampleEmotion with either Neutral, Angry, Fear, Happy, Curious.";
		prompt +=
